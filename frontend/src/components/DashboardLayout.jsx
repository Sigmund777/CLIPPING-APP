      ...
      <aside className="border-r border-white/5 bg-ink-900/40 hidden lg:flex flex-col">
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full bg-purple/20 border border-purple/30 flex items-center justify-center text-xs font-medium text-purple-200">
              {(user?.name || user?.email || "U").slice(0, 1).toUpperCase()}
            </div>
            <div className="min-w-0">
              <div className="text-sm font-medium truncate">{user?.name || user?.email?.split("@")[0]}</div>
              <div className="text-[10px] uppercase tracking-wider text-zinc-500">{user?.email}</div>
            </div>
          </div>
